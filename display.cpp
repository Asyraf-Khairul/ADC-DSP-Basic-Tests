...
constexpr uint32_t kI2cBaudRate = 400000u;

ssd1306_t display;
uint8_t display_buffer[(kDisplayWidth + 1u) * (kDisplayHeight / 8u)];

void write_commands(uint8_t address) {
    ...
    i2c_write_blocking(i2c1, address, commands, sizeof(commands), false);
}
